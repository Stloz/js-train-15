...
function calculateCircleArea(radius) {
  ...
  if (typeof radius !== "number") {
    ...
    return `Полоща кола радіусом ${radius} = ${area}`;
  }
}
...
function calculateHypotenuse(a, b) {
  ...
  return `Довжина гіпотенузи для прямокутного трикутника з катетами довжиною ${a} та ${b}, буде дорівнювати ${c}.`;
}
...
function calculateVolumeCylinder(radius, height) {
  ...
  if (typeof radius !== "number" || typeof height !== "number") {
    console.log("Помилка: Один чи обидва з аргументів не є числом");
    return null;
  }
...
function sumPositiveNumbers(numbers) {
  ...
  return `Сума додатніх чисел = ${sum}`;
}
...
function getFractionalPart(num) {
  ...
  const drib = num - Math.floor(num);
  return `Дробова частина числа = ${drib}`;
}
...
function compareAndRound(num1, num2) {
  ...
  return `Найбільше число округлене до найближчого цілого ${Math.round(
    Math.max(num1, num2)
  )}`;
}
...
function estimateInvestment(principal, interestRate, years) {
  ...
  if (
    ...
    console.log("Помилка: не всі аргументи є числами.");
    return null;
  }
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (!Array.isArray(products)) {
    console.log("Помилка: Аргумент (products) не є масивом");
    return null;
  }
